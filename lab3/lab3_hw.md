...
```
The data are taken from V.M Savage and G.B. West, as presented in the help function window.
...
```
The dimensions of the data frame is 83 rows * 11 columns. Specifically, I used the dim() function showing out the dimensions of "sleep" directly. (str() and glimpse() functions work as well.) An alternative way is to read off from the help window.
...
## [1] TRUE
```
There are NAs in the data, as the result showed in the anyNA() function.
...
7. We are interested in two groups; small and large mammals. Let's define small as less than or equal to 1kg body weight and large as greater than or equal to 200kg body weight. Make two new data frames (large and small) based on these parameters.
...
## [1] 0.2596667
```
The mean weight for the small mammals is around 0.26kg.
...
## [1] 1747.071
```
The mean weight for the large mammals is around 1747.07kg.
...
## [1] 3.3
```
By calculating the average sleep time for both, we see that small animals sleep longer on average.
